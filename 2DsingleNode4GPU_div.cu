...
void BrocastInitState(float* init_data,int width,int height){
    ...
    if(my_rank == 0){
        ...
        ncclSend(buffer+sendcounts[0]+sendcounts[1],sendcounts[2],ncclFloat,2,nccl_comm,s);
        ncclSend(buffer+sendcounts[0]+sendcounts[1]+sendcounts[2],sendcounts[3],ncclFloat,3,nccl_comm,s);
    }
    else{
...
void HaloExchange(){
    ...
    if(y%2 == 0){
        ncclSend(device_data,local_width,ncclFloat,neighbor_up,nccl_comm,s);
        ncclRecv(halo_buf[0],local_width,ncclFloat,neighbor_up,nccl_comm,s);
        // MPI_Sendrecv(data,local_width,MPI_FLOAT,neighbor_up,0,halo_buf[0],local_width,MPI_FLOAT,neighbor_up,0,MPI_COMM_WORLD,NULL);
    }else{
        ncclRecv(halo_buf[2],local_width,ncclFloat,neighbor_down,nccl_comm,s);
        ncclSend(device_data+(local_height-1)*local_width,local_width,ncclFloat,neighbor_down,nccl_comm,s);
    ...
    if(y%2 == 0){
        ...
        ncclRecv(device_data,local_width,ncclFloat,neighbor_up,nccl_comm,s);
        ncclSend(halo_buf[0],local_width,ncclFloat,neighbor_up,nccl_comm,s);
    ...
    if(x%2 == 0){
        ncclSend(halo_buf[4],local_height,ncclFloat,neighbor_left,nccl_comm,s);
        ncclRecv(halo_buf[3],local_height,ncclFloat,neighbor_left,nccl_comm,s);
        ...
        ncclSend(halo_buf[5],local_height,ncclFloat,neighbor_right,nccl_comm,s);
        ncclRecv(halo_buf[1],local_height,ncclFloat,neighbor_right,nccl_comm,s);
    ...
    if(x%2 == 0){
        ...
        ncclSend(halo_buf[4],local_height,ncclFloat,neighbor_right,nccl_comm,s);
        ncclRecv(halo_buf[3],local_height,ncclFloat,neighbor_right,nccl_comm,s);
